...
1 점
채점 설명:
본 논문에서는 AI를 기반으로 한 코딩 에이전트가 보안 측면에서 더 취약한 코드를 생성하는지에 관해 다룹니다. 몇 가지의 프로그래밍 과제를 수행하면서, AI를 사용한 그룹은 AI를 사용하지 않은 그룹에 비해 더 보안적으로 취약한 코드를 작성했으며, 심지어 자신이 작성한 코드가 더 안전하다고 믿었습니다. 이러한 실험 결과를 통해, AI agent 이용 시 생길 수 있는 보안 위협을 주의해야 한다고 저자들은 주장합니다.
Q3 Strengths (at least three items)
1 점
채점 설명:
본 논문은 originality 측면에서 장점이 있습니다. 기존 연구들은 대부분 주어진 과제를 AI 스스로 해결하도록 했을 때의 결과에 초점을 두었다면 해당 방법은 사용자와 AI의 상호작용 초점을 두었습니다. 또, AI agent의 사용이 증가되고 있는 현 시점에서 AI의 강점뿐만 아니라 주의해야 할 점을 부각했다는 점에서 significance가 있습니다. 마지막으로 clarity 측면에서 실험 과정과 디테일을 상세하게 서술하여 재현성과 확장성을 높였습니다.
Q4 Weaknesses (at least three items)
1 점
채점 설명:
Quality 측면에서 부족한 부분이 있습니다. 실험에서 참가자 수는 47명이었는데 이는 실험 결과를 일반화하기에 부족한 숫자입니다. 또, originality 측면에서 코딩에서의 AI agent에 문제 제기를 했다는 점은 참신하지만 AI 자체의 취약점에 대한 기존 연구는 많이 존재합니다. 마지막으로 clarity 측면에서 취약성의 평가 기준이 다소 임의적이고 모호하다는 한계가 있습니다.
Q5 Possible Enhancements (at least three items)
1 점
채점 설명:
먼저 취약성의 판단 기준을 객관적이고 정량화된 방식으로 교체할 수 있습니다. 또, 많은 인원에 대한 실험을 진행하면 연구의 신뢰성을 높일 수 있을 것입니다. 마지막으로 실제로 ai agent를 사용하여 작성된 현 시대의 서비스중인 많은 코드들에 대해 취약성을 찾는다면 significance를 더 높일 수 있을 것입니다.
## 7주차
Q2 Summary (1 paragraph)
1 점
채점 설명:
해당 논문에서는 javascript의 명세와 실제 구현 간의 불일치를 해결하기 위한 자동화된 방식을 제시했습니다. 자바스크립트의 공식 명세는 ECMA라는 문서로 정리되어 있으며 실제로 해당 명세와 구현 간에 맞지 않는 부분이 많이 존재했습니다. 하지만 문서와 코드가 매우 길고 복잡해 이러한 오류를 잡아내는데 어려움이 있었습니다. ESMeta는 이를 자동으로 해결할 수 있었으며 이러한 능력을 인정받아 Javascript의 파이프라인에 통합되기까지 했습니다.
Q3 Strengths (at least three items)
1 점
채점 설명:
해당 논문은 originality 측면에서 장점이 있다. 기존에는 명세와 구현 간의 검증을 수작업으로 진행하였으며 자동화된 시스템이 존재하기 않았기 때문에 해당 논문의 method가 originality를 갖는다. 두번째로 quality 측면에서 실제 javascript 파이프라인에 통합되어 수많은 버그들을 잡아낼 수 있었기 때문에 quality 측면에서의 장점이 있다. 마지막으로 significance 측면에서 자동화된 명세 검증이라는 새로운 개념을 제시했기 때문에 강점이 있다.
Q4 Weaknesses (at least three items)
1 점
채점 설명:
originality 측면에서 해당 논문은 기존의 분석 기술과는 큰 차이가 없다. 명세와 관련해 이러한 기술을 적용한 사례는 없었지만 분석 측면에서는 크게 새롭지 않다. 두 번째로 quality 측면에서 다양한 언어에서의 적용 가능성에 대한 부분이 언급되지 않은 부분이 아쉽다. 마지막으로 clarity 측면에서 해당 기술은 다양한 engineering이 적용되어있기 때문에 논문만으로 내용 전체를 이해하기 힘든 부분이 있다.
Q5 Possible Enhancements (at least three items)
1 점
채점 설명:
해당 논문에서 더 발전할 수 있는 부분으로 먼저 작동하는 방식에 대한 pipeline figure가 있으면 좋을 것 같다. 전체 내용을 한 번에 이해할 수 있을 것이다. 그리고 기존의 분석 기술과 차별화되는 점을 강조하면 좋을 것 같다. 기존 기술을 단순히 새로운 도메인에 적용한 것이 아닌 새로운 측면을 강조하면 좋을 것이다. 마지막으로 조각 코드와 데모를 제작하여 직관적인 이해를 도울 수 있다.
